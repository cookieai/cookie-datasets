 ...
 */
private[spark] trait CachedStatus extends FileInputFormat[String, Row] {
 ...
 */
private[spark] trait PrunedReader extends RecordReader[String,Row] {
...
private[spark] object PrunedReader {
  val REQUIRED_COLUMNS = "ai.cookie.spark.sql.sources.mapreduce.PrunedReader.REQUIRED_COLUMNS"
  ...
  def getRequiredColumns(conf: HadoopConfiguration): Array[String] = {
    Option(conf.getStrings(REQUIRED_COLUMNS)).getOrElse(new Array[String](0))
  }
}
